function plot_all_sgnnoise_realsgn(X_est,betas_est,signal_wn,signal,opts)
...
for i = 1: opts.iter
    H(i) = plot(1:gap:opts.npts*opts.num_runs, signal_wn(1:gap:end,i),'Color',[0.5,0.5,0.5]);hold on; % Fixed for multiples RUNs
end

h2 = plot(1:opts.npts*opts.num_runs,signal,'R','LineWidth',2);
xlabel('TR');
grid on;
ylabel('$\displaystyle\sum\limits_{i} \beta_i Stim_i + \epsilon $','interpreter','latex', 'FontSize',14,'FontWeight','bold');    % LaTex Ylabel
title(h0,'Signals Vs Exp','FontWeight','bold')
legend([H(1),h2],'Signals','Exp');
...
h2 = plot(1:opts.npts*opts.num_runs,signal,'R','LineWidth',2);                                      % Fixed for multiples RUNs
h3 = plot(1:opts.npts*opts.num_runs,mean(signal_est,2),'B','LineWidth',2);  

xlabel('TR');
grid on;

title(h0,'After Estimation: Signals Vs Exp','FontWeight','bold')
legend([H2(1),h2,h3],'Estimated Signals','Exp', 'Mean Estimated Signal');
legend('boxoff');
set(gca,'XLim',[1,opts.npts*opts.num_runs]);                                                        % Setting the X limits
